...
public class PlayerLandsReporter : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision c)
    {
        ...
        if (c.impulse.magnitude > 0.5f)
        {
			...
			EventManager.TriggerEvent<PlayerLandsEvent, Vector3, string>(c.contacts[0].point, GameObject.Find("M1Avatar").GetComponent<YBotSimpleControlScript>().surfaceType);
        }
